...
```{r dockq-boxplot-combined, fig.width=18, fig.height=5, warning=FALSE, message=FALSE, results='hide', echo=FALSE}
plots <- list()
for (f in dockq_files) {
  ...
  if ("dockq" %in% names(df)) {
    ...
    p <- ggplot(df, aes_string(x = x_col, y = "dockq")) +
      ...
      geom_jitter(data = df %>% filter(rank_type == "other"), aes(color = rank_type), width = 0.2, alpha = 0.5) +
      geom_jitter(data = df %>% filter(rank_type == "rank1"), aes(color = rank_type), width = 0.2, size = 3, alpha = 0.9) +
      scale_color_manual(values = c("rank1" = "#E41A1C", "other" = "#377EB8")) +
      labs(title = paste(ds, "DockQ Box Plot"), x = ds, y = "DockQ", color = "Rank") +
      theme_classic() +
      ylim(0, 1) +
