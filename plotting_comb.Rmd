...
```{r dockq-scatter-combined, fig.width=18, fig.height=6.5, warning=FALSE, message=FALSE, results='hide', echo=FALSE}
plots <- list()
...
```{r meandockq-bar-combined, fig.width=18, fig.height=6.5, warning=FALSE, message=FALSE, results='hide', echo=FALSE}
plots <- list()
...
```{r correlation-plots-combined, fig.width=18, fig.height=13, warning=FALSE, message=FALSE, results='hide', echo=FALSE}
pearson_plots <- list()
spearman_plots <- list()
for (f in correlation_files) {
  ...
  if (nrow(df_spearman) > 0) {
    p <- ggplot(df_spearman, aes(x = feature, y = r)) +
      geom_bar(stat = "identity", position = "dodge", fill = "#FFA500") +
      coord_flip() +
