...
Place this YAML header before your first code chunk. This will fix the `object 'params' not found` error.// filepath: /Users/vitran/Documents/Work/Github/benchmark_plotting/plotting_comb.Rmd
---
title: "DockQ Benchmark Report (Combined)"
date: "`r format(Sys.time(), '%d %B, %Y %H:%M:%S')`"
output:
...
library(tidyr)
library(readr)
library(patchwork) # Add this for plot arrangement
...
plots <- list()
for (ds in datasets) {
  csv_file <- file.path(output_dir, paste0(ds, ".dockq.csv"))
...
plots <- list()
for (ds in datasets) {
  csv_file <- file.path(output_dir, paste0(ds, ".meandockq.csv"))
  if (file.exists(csv_file)) {
    df <- read_csv(csv_file)
...
spearman_plots <- list()
for (ds in datasets) {
  csv_file <- file.path(output_dir, paste0(ds, ".correlation.csv"))
  if (file.exists(csv_file)) {
    df <- read_csv(csv_file)
